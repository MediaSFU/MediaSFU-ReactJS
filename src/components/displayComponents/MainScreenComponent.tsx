...
const MainScreenComponent: React.FC<MainScreenComponentOptions> = ({
  ...
  useEffect(() => {
    const handleResize = () => setDimensions(getParentDimensions());
  ...
  const isWideScreen = useMemo(() => {
    ...
    return width > 1.5 * (height || 1);
  }, [dimensions]);

  const calculateComponentSizes = useCallback((): ComponentSizes => {
    const { width: parentWidth, height: parentHeight } = dimensions;

    if (doStack) {
      if (isWideScreen) {
        const mainWidth = Math.floor((mainSize / 100) * parentWidth);
        const otherWidth = Math.floor(((100 - mainSize) / 100) * parentWidth);
      ...
      const mainHeight = Math.floor((mainSize / 100) * parentHeight);
      const otherHeight = Math.floor(((100 - mainSize) / 100) * parentHeight);
  ...
  }, [dimensions, doStack, isWideScreen, mainSize]);
  ...
  const renderableChildren = React.Children.map(children, (child, index) => {
  ...
  const childrenNode = renderChildren
    ? renderChildren({
        defaultChildren: renderableChildren,
        isWideScreen,
        doStack,
        componentSizes,
      })
