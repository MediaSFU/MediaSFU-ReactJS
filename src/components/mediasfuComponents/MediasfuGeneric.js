...
function MediasfuGeneric({ PrejoinPage=WelcomePage, credentials={}, useLocalUIMode = false, seedData = {}, useSeed = false }) {
    ...
    const eventType = useRef(useSeed && seedData?.eventType ? seedData?.eventType : 'broadcast'); // Event type
    ...
    const controlButtons = [
        ...
        {
            icon: faDesktop,
            alternateIconComponent: <DesktopAccessDisabledIcon fontSize="lg" style={{ color: 'red' }} />,
                ...
                <div style={{ position: 'relative' }}>
                    ...
                    {showMessagesBadge && (
                        ...
                        >
                            ...
                            >
                                <span style={{ color: 'white', fontSize: 8, fontWeight: 'bold' }}>
                                </span>
                            </div>
                        </div>
                    )}
    ...
    const computeDimensionsMethod = ({
        ...
        const isWideScreen_ = parentWidth > 768;
    ...
    async function connect_Socket(apiUserName, apiKey, apiToken, link) {
        ...
        if (socket.current.id) {
            ...
            socket.current.on('disconnect', async () => {
                ...
                await disconnect({ parameters: { showAlert, redirectURL: redirectURL.current, onWeb: 'web' == 'web', updateValidated } });
                if (videoAlreadyOn.current) {
                    await clickVideo({ parameters: { ...getAllParams(), ...mediaSFUFunctions(), MediaStream, MediaStreamTrack, mediaDevices, device: device.current, socket: socket.current, showAlert, checkPermission, streamSuccessVideo, hasCameraPermission, requestPermissionCamera, checkMediaPermission: 'web' != 'web' } })
                }
                if (audioAlreadyOn.current) {
                    await clickAudio({ parameters: { ...getAllParams(), ...mediaSFUFunctions(), MediaStream, MediaStreamTrack, mediaDevices, device: device.current, socket: socket.current, showAlert, checkPermission, streamSuccessAudio, hasAudioPermission, requestPermissionAudio, checkMediaPermission: 'web' != 'web' } })
                }
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                ...
                                <ControlButtonsComponentTouch buttons={recordButton} direction={'horizontal'} showAspect={eventType.current == 'broadcast' && !showRecordButtons && islevel.current == '2'} location='bottom' position='middle' />

                                {/* Buttons to control recording */}
                                <ControlButtonsComponentTouch buttons={recordButtons} direction={'horizontal'} showAspect={eventType.current == 'broadcast' && showRecordButtons && islevel.current == '2'} location='bottom' position='middle' />
                            ...
                            >
                                ...
                                <div style={{
                                    width: paginationDirection.current === 'horizontal' ? componentSizes.current.otherWidth : paginationHeightWidth.current,
                                    height: paginationDirection.current === 'horizontal' ? paginationHeightWidth.current : componentSizes.current.otherHeight,
                                    padding: 0,
                                    margin: 0,
                                    display: doPaginate.current ? 'flex' : 'none',
                                    flexDirection: paginationDirection.current === 'horizontal' ? 'row' : 'column',
                                    justifyContent: 'center',
                                    alignItems: 'center',
                                }}>
