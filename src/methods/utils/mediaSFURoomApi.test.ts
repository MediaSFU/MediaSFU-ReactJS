...
const validApiUserName = 'sampleusr';
const validApiKey = 'a'.repeat(64);
const expectedManagedRoomApiUrl = resolveMediaSFURoomApi('', 'createRoom');
...
describe('MediaSFU room helpers', () => {
  ...
  it('creates rooms against the managed API by default and clears the pending marker on success', async () => {
  ...
  it('joins rooms against the managed API by default', async () => {
    fetchMock.mockResolvedValue(mockSuccessResponse());
