...
const LoadingModal: React.FC<LoadingModalOptions> = ({
  ...
  const spinnerElement = showSpinner
    ? (
        <FontAwesomeIcon
          icon={spinnerIcon}
          spin
          style={{ fontSize: "48px", color: displayColor }}
          {...spinnerProps}
        />
      )
    : null;

  const renderedSpinner = renderSpinner
    ? renderSpinner({
        defaultSpinner: spinnerElement,
        isVisible,
        displayColor,
      })
    : spinnerElement;
  ...
  const defaultContentNode = (
    <div className={contentClassNames} style={contentStyle} {...restContentProps}>
      ...
      {loadingText && (
        <div className={textClassNames} style={textStyle} {...restTextProps}>
          {loadingText}
        </div>
      )}
    </div>
