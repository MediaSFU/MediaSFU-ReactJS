...
export interface ConfirmHereModalOptions {
  ...
  renderSpinner?: (options: { defaultSpinner: React.ReactNode }) => React.ReactNode;
  renderTitle?: (options: { defaultTitle: React.ReactNode }) => React.ReactNode;
  ...
  renderConfirmButton?: (options: { defaultButton: React.ReactNode }) => React.ReactNode;
  renderBody?: (options: {
...
const ConfirmHereModal: React.FC<ConfirmHereModalOptions> = ({
  ...
  const initialDuration = useMemo(
    () => Math.max(0, countdownDuration ?? 120),
    [countdownDuration]
  );
  ...
  }, [localSocket, member, roomName, socket]);
  ...
  const handleTimeout = useCallback(() => {
    ...
    onTimeout?.();
    confirmAndClose();
  }, [confirmAndClose, emitDisconnect, onTimeout]);

  useEffect(() => {
    setCounter(initialDuration);
  }, [initialDuration, isConfirmHereModalVisible]);
  ...
  const overlayClassNames = [
    "mediasfu-confirm-here__overlay",
    overlayClassName,
  ]
    .filter(Boolean)
    .join(" ")
  ...
  const overlayStyle: React.CSSProperties = {
    ...
    display: isConfirmHereModalVisible ? "flex" : "none",
    alignItems: "center",
  ...
  const contentClassNames = [
    "mediasfu-confirm-here__content",
    contentClassName,
  ]
    .filter(Boolean)
    .join(" ")
  ...
  const {
    ...
    style: spinnerWrapperStyleOverrides,
    ...restSpinnerWrapperProps
  } = spinnerWrapperProps ?? {};

  const spinnerWrapperClassNames = [
    "mediasfu-confirm-here__spinner",
  ...
  const spinnerWrapperStyle: React.CSSProperties = {
    fontSize: "3rem",
    color: "black",
    ...spinnerWrapperStyleOverrides,
  };

  const {
    style: spinnerIconStyleOverrides,
    ...restSpinnerIconProps
  } = spinnerIconProps ?? {};

  const spinnerIconStyle: React.CSSProperties = {
    fontSize: "inherit",
    color: "inherit",
    ...spinnerIconStyleOverrides,
  };
  ...
  const messageClassNames = [
    "mediasfu-confirm-here__message",
    messageClassName,
  ]
    .filter(Boolean)
    .join(" ")
  ...
  const {
    className: countdownWrapperClassName,
    style: countdownWrapperStyleOverrides,
    ...restCountdownWrapperProps
  } = countdownWrapperProps ?? {};

  const countdownWrapperClassNames = [
    "mediasfu-confirm-here__countdown",
    countdownWrapperClassName,
  ]
    .filter(Boolean)
    .join(" ")
    .trim() || undefined;

  const countdownWrapperStyle: React.CSSProperties = {
    fontSize: "0.95rem",
    color: "black",
    margin: 0,
    ...countdownWrapperStyleOverrides,
  };

  const {
    className: countdownValueClassName,
    style: countdownValueStyleOverrides,
    ...restCountdownValueProps
  } = countdownValueProps ?? {};

  const countdownValueClassNames = [
    "mediasfu-confirm-here__countdown-value",
    countdownValueClassName,
  ]
    .filter(Boolean)
    .join(" ")
    .trim() || undefined;
  ...
  const confirmButtonClassNames = [
    "mediasfu-confirm-here__confirm",
  ...
  const resolvedMessage = useMemo(() => {
    if (typeof message === "function") {
      return message({ counter, countdownDuration: initialDuration });
    }
    return message;
  }, [counter, initialDuration, message]);

  const defaultSpinner = (
    <div
      className={spinnerWrapperClassNames}
  ...
  const spinnerNode = renderSpinner
  ...
  const defaultCountdownNode = (
    <p
      className={countdownWrapperClassNames}
      style={countdownWrapperStyle}
      {...restCountdownWrapperProps}
    >
      {countdownLabel} {" "}
      <span
        className={countdownValueClassNames}
        style={countdownValueStyle}
        {...restCountdownValueProps}
      >
        {counter}
      </span>
  ...
  const countdownNode = renderCountdown
    ? renderCountdown({
        defaultCountdown: defaultCountdownNode,
        counter,
        countdownLabel,
      })
    : defaultCountdownNode;

  const handleConfirmHereClick: React.MouseEventHandler<HTMLButtonElement> = (
    event
  ) => {
  ...
  const confirmButtonNode = renderConfirmButton
    ? renderConfirmButton({ defaultButton: defaultConfirmButton })
  ...
  const bodyNode = renderBody
    ? renderBody({
        ...
        confirmButton: confirmButtonNode,
      })
    : defaultBody;
