    ...
    if (pseudoName) {
      ...
      if (remoteProducerId == 'youyou' || remoteProducerId == 'youyouyou') {
        ...
        if (!videoAlreadyOn) {
          ...
          participant = await { id: 'youyouyou', stream: localStreamVideo, name: 'youyouyou' }
          participant_ = await { id: 'youyou', videoID: 'youyou', name: 'youyouyou', stream: localStreamVideo }
          participant.muted = true
          remoteProducerId = await 'youyouyou'

          await newComponents[0].push(<VideoCard

            videoStream={localStreamVideo ? localStreamVideo : null}
            remoteProducerId={localStreamVideo ? localStreamVideo.id : null}
            eventType={eventType}
