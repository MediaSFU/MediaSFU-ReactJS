...
function MediasfuBroadcast({ PrejoinPage=WelcomePage, credentials={}, useLocalUIMode = false, seedData = {}, useSeed = false }) {
    ...
    const eventType = useRef('broadcast'); // Event type
    ...
    const waitingRoomList = useRef([]); // Waiting room list

    const waitingRoomCounter = useRef(0); // Waiting room counter

    const filteredWaitingRoomList = useRef([]); // Filtered waiting room list
    ...
    const requestList = useRef([]); // Request list

    const requestCounter = useRef(0); // Request counter
    const filteredRequestList = useRef([]); // Filtered request list
